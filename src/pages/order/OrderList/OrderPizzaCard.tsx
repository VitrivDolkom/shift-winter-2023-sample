            ...
            <>
                <div className={s.choice}>
                    <ul className={s.crusts}>
                        <li className={[s.crust, pizzaCard.crust === 'cheesy' ? s.current : ''].join(' ')}>
                            сыр. корка + {pizzaCard.pizza.price.crust?.cheesy} ₽
                        </li>
                        <li className={[s.crust, pizzaCard.crust === 'cheesySausage' ? s.current : ''].join(' ')}>
                            сыр. соус + {pizzaCard.pizza.price.crust?.cheesySausage} ₽
                        </li>
                    </ul>
                    <ul className={s.sized}>
                        <li className={[s.size, pizzaCard.size === 'small' ? s.current : ''].join(' ')}>
                            15 см + {pizzaCard.pizza.price.size.small} ₽
                        </li>
                        <li className={[s.size, pizzaCard.size === 'medium' ? s.current : ''].join(' ')}>
                            20 см + {pizzaCard.pizza.price.size.medium} ₽
                        </li>
                        <li className={[s.size, pizzaCard.size === 'large' ? s.current : ''].join(' ')}>
                            30 см + {pizzaCard.pizza.price.size.large} ₽
                        </li>
                    </ul>
                </div>
                <div className="price">{pizzaCard.quantity}</div>
            </>
        )}
        {!success && (
            <>
                ...
                <div className={s.actions}>
                    <div className={s.quantityActions}>
                        <div
                            onClick={() => callbacks.decreaseQuantity(pizzaCard.pizza.id)}
